/* recipe-data.js - holds the recipe data used by script.js */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderRecipes([getRandomRecipe(recipes)], recipeSection);

    searchForm.addEventListener('submit', (event) => {
        event.preventDefault();

        const keyword = searchInput.value.trim().toLowerCase();
        const matchingRecipes = filterRecipes(recipes, keyword);
        const sortedRecipes = sortRecipesByName(matchingRecipes);

        renderRecipes(sortedRecipes, recipeSection);
    });
});
...
function filterRecipes(recipeArray, keyword) {
...
function sortRecipesByName(recipeArray) {
    return [...recipeArray].sort((a, b) => a.name.localeCompare(b.name));
}


function buildStarRating(rating) {
    const starWrapper = document.createElement('span');
    starWrapper.className = 'rating';
    starWrapper.setAttribute('role', 'img');
    starWrapper.setAttribute('aria-label', `Rating: ${rating} out of 5 stars`);
...
function buildRecipeCard(recipe) {
    ...
    imgWrap.className = 'recipe-img-wrap';
    ...
    body.appendChild(buildTagList(recipe.tags));
    body.appendChild(title);
    body.appendChild(buildStarRating(recipe.rating));
...
function renderRecipes(recipeArray, recipeSection) {
    recipeSection.replaceChildren();

    if (recipeArray.length === 0) {
        const noResults = document.createElement('p');
        noResults.className = 'no-results';
        noResults.textContent = 'No recipes matched your search. Try another keyword.';
        recipeSection.appendChild(noResults);
        return;
    }

    recipeArray.forEach((recipe) => {
        recipeSection.appendChild(buildRecipeCard(recipe));
    });
}